...
function encriptar_frase(frase, input, output) {
  ...
  var check = frase.split("").map(function (item) {
    if (item == " ") {
      return item;
    } else {
...
function saltos_sucesivos(alphabet, llave) {
...
function saltos_idi(alphabet, key) {
  var start = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  key.split("").map(function (item) {
    if (isNaN(item)) varters.push(item);else numbers.push(Number(item));
  });
  ...
  for (var i = 0; i < starterInputSize; i++) {
    ...
    if (start) {
      start = false;
    } else if (currentvarter === "I" || currentvarter === "i") {
      direction = -1;
    } else if (currentvarter === "D" || currentvarter === "d") {
      direction = 1;
    } else {
...
function desencriptar() {
  var frase2 = document.querySelector("#frase").value.toLowerCase();
